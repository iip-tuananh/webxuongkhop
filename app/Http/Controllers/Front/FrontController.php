...
use App\Model\Admin\ProductBanner;
use App\Model\Admin\Project;
use App\Model\Admin\Rate;
use App\Model\Admin\RegisterBlock;
...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['news'] = Post::query()->with(['image', 'user_create'])
            ->where('type', 'post')
            ->latest()->get()->take(3);
        $data['about'] = About::query()->with(['image'])->first();
        $data['services'] = Service::query()->with(['image'])->where('status', 1)->get();
        $data['treatmentSteps'] = TreatmentStep::query()->with(['image'])->orderBy('step')->get();
        $data['categoriesSpecial'] = CategorySpecial::query()->with([
            'products' => function ($q) {
        ...
        $data['registerBlock'] = RegisterBlock::query()->with(['image'])->first();
        $data['reviewsVideo'] = Review::query()->where('type', 'video')->get();
        $data['reviewsText'] = Review::query()->with('image')->where('type', 'text')->get();
    ...
    public function abouts(Request $request) {
        ...
        $partners = Partner::query()->with(['image'])->orderBy('created_at', 'asc')->get();

        return view('site.about_us', compact('config', 'reviews', 'partners'));
    }
    ...
    public function services(Request $request, $parentSlug, $slug = null) {
        $category = null;
        $categoryParent = null;

        if($slug) {
            $category = PostCategory::query()->where('slug', $slug)->first();
            $categoryParent = PostCategory::query()->where('id', $category->parent_id)->first();
            $services = Service::query()->with(['image', 'user_create'])->where('status', 1)
                ->where('cate_id', $category->id)
                ->latest()->get();
        } else {
            $categoryParent = PostCategory::findBySlug($parentSlug);
            $cateIds = $categoryParent->childs()->pluck('id')->toArray();
            $services = Service::query()->with(['image', 'user_create'])->where('status', 1)
                ->whereIn('cate_id', $cateIds)
                ->latest()->get();
        }

        $allCategoryServices = PostCategory::query()->with(['image'])
            ->where('type', PostCategory::TYPE_SERVICE)
            ->where('parent_id', 0)
            ->latest()->get();

        return view('site.services', compact('services', 'category', 'categoryParent', 'allCategoryServices'));
    }

    public function getServiceDetail(Request $request, $slug) {
        $service = Service::findBySlug($slug);
        $allServices = Service::query()->latest()->get();

        return view('site.service_detail', compact('service', 'allServices'));
    }

    public function getProducts(Request $request) {
        $products = Product::query()->with('image')->where('status', 1)->latest()->get();
        $banner = ProductBanner::query()->with(['image'])->first();

        return view('site.product_category', compact('products', 'banner'));
    }

    public function getProductDetail(Request $request, $slug = null) {
        $product = Product::query()->with(['image', 'galleries.image'])->where('slug', $slug)->first();
        $reviews = Rate::query()->where(['product_id' => $product->id, 'status' => 2])->latest()->get();

        return view('site.product_detail', compact('product', 'reviews'));
    }
    ...
    public function getProjectDetail(Request $request, $slug) {
        ...
        $projects = Project::query()->where('status', 1)->whereNotIn('id', [$project->id])->latest()->get()->take(3);

        return view('site.project_detail', compact('project', 'projects'));
    }

    public function blogs(Request $request) {
        $posts = Post::query()->with(['image'])
            ->where(['type' => 'post', 'status' => 1])
            ->latest()->paginate(12);

        $newPosts = Post::query()->with(['image'])
            ->where(['type' => 'post', 'status' => 1])
            ->latest()->take(5)->get();
        $title = 'Tin tức';

        return view('site.blog', compact('posts', 'newPosts', 'title'));
    }
    ...
    public function blogDetail(Request $request, $slug) {
        ...
        return view('site.blog_detail', compact('blog', 'newPosts', 'title'));
    }

    public function hirings(Request $request) {
        $posts = Post::query()->with(['image'])
            ->where(['type' => 'hiring', 'status' => 1])
            ->latest()->paginate(12);
        $newPosts = Post::query()->with(['image'])
            ->where(['type' => 'hiring', 'status' => 1])
            ->latest()->take(5)->get();
    ...
    public function hiringDetail(Request $request, $slug) {
        $blog = Post::query()->with(['image', 'user_create'])->where('slug', $slug)->first();
        $newPosts = Post::query()->with(['image'])
            ->where(['type' => 'hiring', 'status' => 1])
            ->latest()->take(5)->get();
...
//            'formReview.content'  => 'required',
        ...
        $validate = Validator::make(
            ...
            [
                ...
                'formReview.title.required' => 'Vui lòng nhập tiêu đề',
                'formReview.address.required' => 'Vui lòng nhập dịa chỉ',
                'formReview.phone.required' => 'Vui lòng nhập số điện thoại',
