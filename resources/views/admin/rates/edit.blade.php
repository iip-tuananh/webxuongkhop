...
<script>
    app.controller('EditReview', function ($rootScope, $scope, $http) {
        $rootScope.$on("editReview", function (event, data){
            ...
            $scope.form = new Rate(data, {scope: $scope});
        ...
        @include('admin.rates.formJs');
        $scope.statues = @json(\App\Model\Admin\Rate::STATUSES);

        // Submit Form sửa
        $scope.submit = function () {
            let url = "/admin/rates/" + $scope.form.id + "/update";
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        $scope.form = new Rate({}, {scope: $scope});
